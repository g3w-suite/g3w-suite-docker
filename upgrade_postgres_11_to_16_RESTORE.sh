...
for DB in "${DBS[@]}"; do
  ...
  echo "psql $CONNECTION_DB -d ${RESTORE_CONNECTION_DBNAME} -c \"create database ${DB}_${SUFFIX_DB};\"" >> pg_restore.sh
  ...
  # Close connection
  echo "psql ${CONNECTION_DB} -d ${RESTORE_CONNECTION_DBNAME} -c \"${QUERY_CLOSE_CONNECTIONS}'${DB}';\"" >> pg_restore.sh

  # Drop old database adn rename the new with old
  echo "psql ${CONNECTION_DB} -d ${RESTORE_CONNECTION_DBNAME} -c \"drop database ${DB};\"" >> pg_restore.sh
  echo "psql ${CONNECTION_DB} -d ${RESTORE_CONNECTION_DBNAME} -c \"alter database ${DB}_${SUFFIX_DB} rename to ${DB};\"" >> pg_restore.sh
